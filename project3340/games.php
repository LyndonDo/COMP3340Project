...
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) { // checking if there is valid game id
    die("<h1>Invalid game ID</h1>");
}

$id = (int)$_GET['id'];

$query = "SELECT * FROM games WHERE id = $id LIMIT 1"; // Get Query From "games" Database
...
$game = mysqli_fetch_assoc($result); // Get Results For The Game

if (!$game) { // Get Results For The Game
    die("<h1>Game not found for ID: $id</h1>");
}
    ...
    <main>
      ...
      <section class="games-container">
        ...
        <?php while ($row = mysqli_fetch_assoc($result)): ?>
            <a href="gamedetail.php?id=<?= urlencode($row['id']) ?>">
        ...
        <a href="gamedetail.php?id=<?= urlencode($row['id']) ?>">
          <img src="images/<?= htmlspecialchars($row['cover_image']) ?>" alt="<?= htmlspecialchars($row['title']) ?>">
        </a>
