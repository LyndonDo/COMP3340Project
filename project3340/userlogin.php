<?php 
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      if(!empty($username) && !empty($password) && !is_numeric($username)) { // checking if the username and password are empty, and checking if the username is not numerical
        $query = "select * from users where username = '$username' limit 1"; // Get Query For User Id, Username, and Password
        
        $result = mysqli_query($connect, $query); 

        if($result ){ // checking the results
          if($result && mysqli_num_rows($result) > 0){ // Checking if there are results

              $user_data = mysqli_fetch_assoc($result);

              if($user_data["password"] === $password) { // Checking if the password is the same as the one store in the database
    ...
    <div class="form-wrapper" style="margin-top: 50px;">
        <div class="form-container">
            <div class="register-login">
                <a href="user_registration.php"><h3>Sign-In</h3></a>
                <a href="userlogin.php"><h3 class="active">Log-In</h3></a>
            </div>
            <form method="post">
                ...
                <div class="selection-container">
                    <input class="btn" type="submit" name="login-in" value="Login-In">
                </div>
            </form>
        </div>
    </div>
